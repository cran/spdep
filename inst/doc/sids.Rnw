...
plot(subset(sidspolys, ((ch$pmap < 0.05) & (ch$type))),
 col=grey(5/7), add=TRUE)
plot(subset(sidspolys, ((ch$pmap < 0.05) & (!ch$type))),
 col=grey(2/7), add=TRUE)
@
...
plot(sidspolys, col=cols[findInterval2(pmap$pmap, brks)])
...
plot(sidspolys, col=cols[findInterval2(stdres, brks)])
legend(c(-84,-81), c(33.9,34.5), fill=cols, legend=leglabs(brks),
 bty="n", ncol=2)
@
...
legend(c(-84,-81), c(33.9,34.5), fill=cols, legend=leglabs(brks),
 bty="n", ncol=2)
plot(subset(sidspolys, sub), col=cols[findInterval2(res$est[sub]*1000, brks)], add=TRUE)
