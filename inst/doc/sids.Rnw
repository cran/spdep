...
directly from listings in the sources refered to above. The county ID
variable is included in both sources\footnote{prior to R 1.9, the underscore in the DBF file feild name \code{CNTY\_ID} is converted to a point, \code{CNTY.ID}.}, and is also the \code{region.id}
...
frame is ordered by ascending county IDs (as are the polygons). To
